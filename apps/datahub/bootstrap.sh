...
mkdir -p "$DEST"
SRC_COMPOSE="$(dirname "$0")/docker-compose.yml"
if [ "$(readlink -f "$SRC_COMPOSE" 2>/dev/null)" != "$(readlink -f "$DEST/docker-compose.yml" 2>/dev/null)" ]; then
  cp "$SRC_COMPOSE" "$DEST/docker-compose.yml"
fi
...
PAYLOAD=$(jq -n \
  ...
  --arg front "http://${PRIVATE_IP}:9002" \
